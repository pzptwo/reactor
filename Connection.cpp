...
Connection::Connection(EventLoop *loop,Socket *clientsock):loop_(loop),clientsock_(clientsock)
{
    ...
    clientchannel_->setcloseback(std::bind(&Connection::closecallback,this));//这里的是在Tcpserver回调
...
void Connection::setslovecb(std::function<void(Connection*,std::string )>fn)
{
    ...
    while(true)
    {
        ...
        else if(nread==-1&&((errno == EAGAIN) || (errno == EWOULDBLOCK)))
        {
            ...
            while(true)
            {
                ...
                /////////////////////////////////
                cout << "recv from client(eventfd=" <<fd() << "): " << message.data() << endl;
                //服务器对信息进行处理,数据的计算
                /*
